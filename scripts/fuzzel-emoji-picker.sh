...
# bemoji-inspired emoji picker using fuzzel as the menu backend.

bm_version=0.4.0-fuzzel
...
usage() {
    echo "Usage: $(basename "$0") [-t | -c | -e] [-f <filepath>] [-p] [-P <number>] [-D <choices>]" 1>&2
    echo
    echo "A simple emoji picker using fuzzel."
    ...
    echo "  -D, --download <choice>    Choices: all|none|emoji|math|nerd."
    echo "  -f, --file <filepath>      Use custom emoji list file or URL."
    echo "  -v, --version              Display version and active paths."
...
_picker() {
    ...
    if command -v fuzzel >/dev/null 2>&1; then
        fuzzel --dmenu --prompt "🔍 " --width 80
        return
    fi

    msg "fuzzel is not installed."
    exit 1
}
